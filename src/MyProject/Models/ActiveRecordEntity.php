...
abstract class ActiveRecordEntity implements \JsonSerializable
{
    ...
    {
        $db = Db::getInstance();
        return $db->query('SELECT * FROM `' . static::getTableName() . '` ORDER BY id DESC LIMIT 5;', [], static::class);
    }
    ...
    public static function deleteArticleComments(string $columnName, $value): ?array
    {
        $db = Db::getInstance();
        $result = $db->query(
            'DELETE FROM `' . static::getTableName() . '` WHERE `' . $columnName . '` = :value;',
            [':value' => $value],
            static::class
        );
        if ($result === []) {
